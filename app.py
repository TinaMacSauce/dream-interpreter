...
WORKSHEET_NAME = "Sheet1"  # <-- your exact tab name
GCRED_PATH = os.getenv("GSPREAD_CREDENTIALS", "/etc/secrets/credentials.json")

# Allow-list for CORS (comma-separated env var). Include all real origins.
ALLOWED_ORIGINS = [
    o.strip() for o in os.getenv(
        "ALLOWED_ORIGINS",
        "https://jamaicantruestories.com,https://www.jamaicantruestories.com,https://plqwhd-jm.myshopify.com,https://admin.shopify.com"
    ).split(",") if o.strip()
]
...
def interpret():
    try:
        ...
        # Accept both keys from different frontends
        user_text = (
            payload.get("dream_text")
            or payload.get("text")
            or (request.form.get("dream_text") or request.form.get("text"))
        )
