# app.py — Jamaican True Stories Dream Interpreter (Phase 2.9.0 — QUICK DECODE + FULL INTERPRETATION)
# Adds:
# - "Full Interpretation" narrative layer (doctrine-safe; built only from matched symbols + sheet fields)
# - Returns `full_interpretation` alongside existing one-liners
# Keeps:
...
def build_full_interpretation_from_doctrine(matches: List[Tuple[Dict, int, Optional[Dict[str, str]]]]) -> str:
    ...
    # Symbol translation: only what your sheet says, but in flowing sentences
    symbol_sentences: List[str] = []
    for sym, mean in zip(symbols, meanings):
        if mean:
            symbol_sentences.append(f"{sym} points to {mean.rstrip('.').strip().lower()}.")

    # Effects: optional
    effect_sentences: List[str] = []
    for eff in effects:
        if eff:
            effect_sentences.append(f"This can show up as {eff.rstrip('.').strip().lower()} in the natural realm.")

    # Actions: combine + dedupe
    action_lines = _dedupe_preserve_order([a for a in actions if a])
    action_text = ""
    if action_lines:
        # Make it read like one instruction block.
        if len(action_lines) == 1:
            action_text = f"What to do: {action_lines[0].rstrip('.') }."
        else:
            joined = " Then, ".join([a.rstrip(".").strip() for a in action_lines])
            action_text = f"What to do: {joined}."
    ...
    if symbol_sentences:
        parts.append(" ".join(symbol_sentences))

    if effect_sentences:
        parts.append(" ".join(effect_sentences))
...
def interpret():
    ...
    if not matches:
        # Keep your existing behavior, but add full_interpretation (empty)
        return jsonify({
            "access": "free",
            "is_paid": False,
            "free_uses_left": 3,
    ...
    # NEW: Full Interpretation (narrative layer) - doctrine safe
    ...
    return jsonify({
        ...
        "free_uses_left": 3,
        "seal": seal,
        "interpretation": interpretation,  # Quick Decode (one-liners)
        "full_interpretation": full_interpretation,  # Full Interpretation (narrative)
        "receipt": {
            "id": receipt_id,
