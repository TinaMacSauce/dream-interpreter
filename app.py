...
# Semantic flags (optional; load heavy deps only if enabled)
...
def interpret():
    # Flask-CORS will auto-handle OPTIONS and add the headers.
    if request.method == "OPTIONS":
        return ("", 204)

    data = request.get_json(silent=True) or {}
    dream = (data.get("dream") or data.get("text") or
             request.form.get("dream") or request.form.get("text") or "").strip()
    if not dream:
