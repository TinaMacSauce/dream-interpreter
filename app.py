...
# and Level 2 AI combiner using Google Sheets as the knowledge base.
...
OPENAI_API_KEY    = os.environ.get("OPENAI_API_KEY", "").strip()  # NEW

if STRIPE_SECRET_KEY:
    stripe.api_key = STRIPE_SECRET_KEY

# OpenAI client (may be None if key missing)
if OPENAI_API_KEY:
    openai_client = OpenAI(api_key=OPENAI_API_KEY)
    logging.info("OpenAI client initialized.")
else:
    openai_client = None
    logging.warning("OPENAI_API_KEY not set; falling back to simple compose_output.")
...
def find_best_rows_for_dream(dream_text: str, max_hits: int = 8) -> List[Dict]:
    ...
    Rules:
      ...
      3. Return up to max_hits rows sorted by score (desc).
    """
    ...
    # Otherwise, pick rows sorted by score
    if keyword_matches:
...
# ── Parse "output" into three sections (legacy/fallback) ────────────
...
def ai_combine_interpretation(dream_text: str, candidate_rows: List[Dict]) -> Dict[str, str]:
    ...
    """
    if not openai_client or not candidate_rows:
        # no AI available or no rows
        logging.warning("ai_combine_interpretation: OpenAI client missing or no candidates; using fallback.")
        # Fallback: use first row via split_output
        ...
        return split_output_to_sections(candidate_rows[0].get("output", ""))
    ...
    try:
        ...
        # Fallback to simple first-row parsing if AI fails
        return split_output_to_sections(candidate_rows[0].get("output", ""))
...
def build_level2_result(dream_text: str, rows: List[Dict]) -> Dict[str, object]:
    ...
    Falls back to compose_output if no rows or AI fails silently.
    """
    if not rows:
...
def stripe_success():
    ...
    # Where to send users back to interpret dreams
