...
# (SSE + Level 4 Seal Engine + Level 5 Formatter + Airtable Level 5 Storage)
#
# Features:
...
# Optional OpenAI client (used only for older "combiner" function)
try:
...
def format_interpretation_payload(result: Dict[str, object]) -> Dict[str, object]:
    ...
    if isinstance(receipt, dict):
        ...
        if "receipt_id" in receipt:
            receipt["receipt_id"] = format_text(receipt.get("receipt_id", ""), 90).replace(" ", "")
...
def session_bootstrap():
    # Session is now just a light cache (Airtable is source of truth when email exists)
    session.setdefault("user_email", "")
    session.setdefault("is_paid", False)
    session.setdefault("free_used", 0)  # fallback when no email
...
def build_receipt(seal: Optional[Dict[str, object]], symbols: List[str]) -> Dict[str, object]:
    ...
    return {
        "receipt_id": "r_" + secrets.token_urlsafe(10),
...
def interpret():
    ...
    # Usage source:
    # - If we have an email: Airtable is source of truth
    # - Else: fall back to session counter
    if email:
        try:
            usage = get_usage(email)
        except Exception:
            usage = {"free_used": int(session.get("free_used", 0)), "email_captured": True}
        free_used = int(usage.get("free_used", 0))
        email_captured = bool(usage.get("email_captured", True))
    else:
    ...
    if is_paid_user:
        ...
        # Save receipt to Airtable (best-effort)
        try:
    ...
    if free_used < FREE_QUOTA:
        ...
        # Save receipt to Airtable if we have an email (best-effort)
        try:
            if email:
                save_receipt(receipt, email=email)
        except Exception:
    ...
    # Save “receipt-only” event too (best-effort)
    try:
        save_receipt(receipt, email=email)
    except Exception:
        pass
...
def create_checkout():
    ...
    Supports GET (link click) and POST (fetch/ajax).
    """
