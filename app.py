# app.py (stable baseline with improved matching + clear meta)
...
def _substring_pick(t: str, inputs: List[str], min_len: int = 6) -> Optional[int]:
    ...
    if not cand:
        return None
    # Prefer longer (more specific) and closer by similarity
...
def find_best_match(user_input: str) -> Tuple[Optional[str], dict]:
    ...
    # 0) Substring containment (precise when your sheet phrase literally appears)
    ...
    # 3) Fuzzy (best overall ratio, require fairly high bar)
    ...
    # 4) Keywords with deterministic tie-break: score → similarity → longer phrase
    scored = []
