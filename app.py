...
# dream text normalizer, and TOP-5 symbol selection (no duplicates).
...
def find_best_rows_for_dream(dream_text: str, max_hits: int = 5) -> List[Dict]:
    ...
    Behavior:
      ...
      - Otherwise, sort scored rows and return TOP max_hits UNIQUE symbols.
    """
...
def ai_combine_interpretation(dream_text: str, candidate_rows: List[Dict]) -> Dict[str, str]:
    ...
    system_message = (
        ...
        "If any symbol relates to danger, spiritual attack, spiritual spouse, or monitoring spirits, "
...
def interpret():
    ...
    if email and is_pro(email):
        rows = find_best_rows_for_dream(text)  # TOP 5 by default
    ...
    rows = find_best_rows_for_dream(text)  # TOP 5 symbols
    result = build_level2_result(text, rows)
