# app.py (stable baseline with improved matching + fallbacks)
...
def _substring_pick(t: str, inputs: List[str], min_len: int = 6) -> Optional[int]:
    """Pick the most specific sheet phrase contained in the user text."""
    cand = [(i, s) for i, s in enumerate(inputs) if len(s) >= min_len and (s in t or t in s)]
    if not cand:
        return None
    cand.sort(key=lambda x: len(x[1]), reverse=True)  # longest (most specific)
    return cand[0][0]

def _jaccard_pick(t: str, inputs: List[str], thresh: float = 0.65) -> Optional[int]:
    """Token Jaccard similarity as a safer middle ground."""
    T = _token_set(t)
    best = (-1.0, None)
...
def find_best_match(user_input: str) -> Tuple[Optional[str], dict]:
    ...
    # 0) Substring containment (precise, e.g. “i was riding a bicycle”)
    sub_idx = _substring_pick(t, inputs)
    if sub_idx is not None:
        return DF.iloc[sub_idx]["output"], {"method": "substring", "match": inputs[sub_idx]}
    ...
    # 2) Token Jaccard (robust for short phrases)
    jac_idx = _jaccard_pick(t, inputs, thresh=0.65)
    if jac_idx is not None:
        return DF.iloc[jac_idx]["output"], {"method": "jaccard", "match": inputs[jac_idx]}

    # 3) Fuzzy (pick best ratio, require fairly high bar)
    ...
    if best_idx is not None and best_ratio >= 0.84:
        return DF.iloc[best_idx]["output"], {"method": "fuzzy", "ratio": round(best_ratio, 4), "match": inputs[best_idx]}
    ...
    if winners:
        ...
        s, idx = winners[0]
        return DF.iloc[idx]["output"], {"method": "keywords", "score": int(s)}
    ...
    if not DISABLE_SEMANTIC:
        ...
        if sem_text and score >= SEMANTIC_SOFT:
