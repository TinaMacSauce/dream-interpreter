...
class Config:
    ...
    AI_NARRATION_TIMEOUT_SECONDS = int(
        os.getenv(
    ...
    AI_NARRATION_MAX_INPUT_CHARS = int(
        os.getenv(
    ...
    AI_NARRATION_MAX_OUTPUT_CHARS = int(
        os.getenv(
    ...
    AI_NARRATION_TEMPERATURE = float(
        os.getenv(
    ...
    def validate(cls):
        ...
        if cls.MIN_DREAM_LENGTH < 1:
            raise RuntimeError(
                "MIN_DREAM_LENGTH must be at least 1."
            )
        ...
        if cls.BASE_MATCH_TOP_K < 1:
            raise RuntimeError(
                "BASE_MATCH_TOP_K must be at least 1."
            )

        if cls.MAX_RULE_HITS_PER_LAYER < 1:
            raise RuntimeError(
                "MAX_RULE_HITS_PER_LAYER must be at least 1."
            )

        if cls.FREE_TRIES < 0:
            raise RuntimeError(
                "FREE_TRIES cannot be negative."
            )

        if cls.NARRATIVE_MAX_SYMBOLS < 1:
            raise RuntimeError(
                "NARRATIVE_MAX_SYMBOLS must be at least 1."
            )

        if cls.NARRATION_MAX_SYMBOLS < 1:
            raise RuntimeError(
                "NARRATION_MAX_SYMBOLS must be at least 1."
            )
        ...
        if cls.AI_NARRATION_MAX_INPUT_CHARS < 500:
            raise RuntimeError(
                "AI_NARRATION_MAX_INPUT_CHARS is set too low."
            )

        if cls.AI_NARRATION_MAX_OUTPUT_CHARS < 200:
            raise RuntimeError(
                "AI_NARRATION_MAX_OUTPUT_CHARS is set too low."
            )
