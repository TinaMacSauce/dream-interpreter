...
from flask import Flask, request, jsonify, session, redirect   # <-- session + redirect added
...
# Flask app
app = Flask(__name__)
app.secret_key = os.environ.get("FLASK_SECRET_KEY", "change-this-secret")  # needed for session
CORS(app)
...
def interpret():
    ...
    # Check PRO via email OR session flag
...
def create_checkout():
    ...
    try:
        session_obj = stripe.checkout.Session.create(
            ...
            # success_url: back to our backend so we can verify and unlock
...
def stripe_success():
    ...
    if email and (status == "complete" or payment_status == "paid"):
        # Persist PRO by email
        make_pro(email)
        # Also unlock for this browser session (no email required afterward)
        session["is_pro"] = True
...
# Optional static success route (can be used as Google Ads 'confirmation page' URL)
@app.get("/success")
def generic_success():
