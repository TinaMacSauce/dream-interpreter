...
from app.snake_registry import (
    ...
    LEGACY_AUTHORITY,
    LEGACY_DOCTRINE_VERSION,
    LEGACY_IMPLEMENTATION_KEYS,
    LEGACY_UPDATED_AT_UTC,
...
def snake_registry_values():
    ...
    for key, (rule_id, status, active) in EXPECTED_RULES.items():
        ...
        version = LEGACY_DOCTRINE_VERSION if legacy else EXPECTED_DOCTRINE_VERSION
        authority = LEGACY_AUTHORITY if legacy else EXPECTED_AUTHORITY
        updated_at = LEGACY_UPDATED_AT_UTC if legacy else EXPECTED_UPDATED_AT_UTC
        values.append([
            rule_id, version, "Snake", f"trigger {key}",
            f"meaning {key}", f"precedence {key}", f"boundary {key}",
            status, authority, version, key,
            updated_at, "TRUE" if active else "FALSE",
        ])
    return values


class SnakeRegistryRuntimeTests(unittest.TestCase):
    def test_append_only_29_rule_manifest_is_verified(self):
        ...
        self.assertEqual(29, snapshot["rule_count"])
        self.assertEqual(24, snapshot["active_rule_count"])
        self.assertEqual(5, snapshot["unresolved_rule_count"])
        self.assertEqual("DoctrineRegistry!A25:M54", snapshot["sheet_range"])
