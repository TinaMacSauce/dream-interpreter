...
def normalize_dream_text(text: str) -> str:
    ...
    for s in starters:
        if lower.startswith(s):
            # strip off the starter and leading spaces
...
def find_best_rows_for_dream(dream_text: str, max_hits: int = 8) -> List[Dict]:
    ...
    Rules:
      1. Normalize dream text (remove 'I was', etc).
      2. If the dream exactly equals a symbol (input), return that row only.
      3. Otherwise, score each row based on:
         - if full symbol phrase inside dream → +4
         - multi-word keyword inside dream   → +3
         - single keyword inside dream       → +1
      4. Return up to max_hits rows sorted by score (highest first).
    """
    # Normalize first so "I was standing..." → "standing..."
    ...
    for row in SHEET_ROWS:
        ...
        # Boost if the full symbol phrase appears in the dream
    ...
    # Take top N rows sorted by score (highest first)
    if keyword_matches:
    ...
    for row in rows:
        ...
        if symbol:
            ...
            if key in seen_symbols:
                continue  # avoid duplicate symbol repeats
...
def ai_combine_interpretation(dream_text: str, candidate_rows: List[Dict]) -> Dict[str, str]:
    ...
    If OpenAI is unavailable or errors (e.g. insufficient_quota), fall back to
    combining ALL candidate_rows using compose_output(), so multi-symbol dreams
    still work without AI.
    """
    # No rows at all → default gentle message
    ...
    # If there is no OpenAI client or key, use multi-row fallback
    ...
    # Build symbols block for AI call
    symbols_block = []
    ...
    try:
        ...
        # Fallback: combine ALL rows using your own logic, not just the first
        fallback = compose_output(candidate_rows)
        return fallback["interpretation"]
...
def interpret():
    ...
    # Normalize dream text (strip "I was", etc.)
...
<body>
  <div class="jts-card">
    ...
    # Payment not completed or no email
