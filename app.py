...
# Optional Sentry (turn on by setting SENTRY_DSN env var)
try:
    import sentry_sdk
...
CORS(app, resources={r"/*": {"origins": __ALLOWED}}) if __ALLOWED else CORS(app)
...
DISABLE_SEMANTIC = os.getenv("DISABLE_SEMANTIC", "0") == "1"         # set to 1 to skip semantic
SEMANTIC_CUTOFF  = float(os.getenv("SEMANTIC_CUTOFF", "0.60"))       # cosine cutoff for semantic
...
# ---------- Sheet Loader (+ snapshot fallback) ----------
def load_sheet(force: bool = False):
    """Load Google Sheet into memory; on failure, fall back to local snapshot if present."""
    global DF, LAST_LOAD
...
def build_embeddings(df: pd.DataFrame):
    """Compute & cache sentence embeddings for normalized inputs."""
    ...
    try:
        ...
        mat = EMB_MODEL.encode(texts, convert_to_numpy=True, normalize_embeddings=True)
        EMB_MATRIX = mat  # already L2-normalized
        logging.info("🔎 Semantic index built: %d vectors", mat.shape[0])
    except Exception as e:
        logging.warning("Semantic disabled (build failed): %s", e)
        EMB_MATRIX = None

def semantic_search(query_norm: str, topk: int = 1):
    """Return best semantic match above cutoff, else None."""
    global EMB_MODEL, EMB_MATRIX
    try:
